        ...
        public virtual Ray CalculateRay()
        {
            ...
            switch (direction)
            {
                case Direction.ViewportCenter:
                    {
                        ...
                        Camera playerCam = playerCamera.GetCamera();
                        Ray viewportRay = playerCam.ViewportPointToRay(ViewportCenter);
                        ...
                        if (Physics.Raycast(viewportRay, out RaycastHit hitInfo, GetDirectionRange(), GetDirectionLayer(), QueryTriggerInteraction.Ignore))
                        {
                            ...
                            // Assign direction only if dot product is positive
                            if (dot > 0)
                            {
                ...
                case Direction.TransformForward:
                    {
                        Vector3 forward = shootDirection.forward;
                        ray.direction = forward;

                        // Single raycast
                        if (Physics.Raycast(shootDirection.position, forward, out RaycastHit hitInfo, GetDirectionRange(), GetDirectionLayer(), QueryTriggerInteraction.Ignore))
                        {
                            Vector3 direction = hitInfo.point - shootPoint.position;
